...
function ArticleBody({ article, locale, t }: { article: Article; locale: string; t: ReturnType<typeof useTranslations> }) {
    ...
    const [advocateData, setAdvocateData] = useState<Record<string, { photo: string; username: string }>>({});
    ...
    useEffect(() => {
        const fetchPhotos = async () => {
            ...
            if (article.advocates && article.advocates.length > 0) {
                const data: Record<string, { photo: string; username: string }> = {};
                await Promise.all(article.advocates.map(async (adv) => {
                    if (adv.userId) {
                        try {
                            ...
                            if (res.data.success) {
                                data[adv.userId] = {
                                    photo: res.data.data.profilePicture || "",
        ...
        <div>
                ...
                <div className="flex items-center gap-4 mb-8 p-5 bg-gray-50 rounded-2xl border border-gray-100 hover:border-gray-200 transition-colors">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                            {authorUsername ? (
                                <Link href={`/profile/${authorUsername}`} className="hover:text-[#C9A227] transition-colors">
            ...
            {article.thumbnail && (
                <div className="relative w-full h-[450px] mb-8 rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                    <Image src={getSafeImageUrl(article.thumbnail)} alt={displayTitle} fill priority={false} unoptimized sizes="(max-width: 1024px) 100vw, 800px" className="object-cover" />
                </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 mb-10 py-3 px-6 bg-white rounded-full border border-gray-200 w-fit mx-auto sm:mx-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleShare("twitter")} className="w-10 h-10 flex items-center justify-center rounded-full bg-[#0A2342] text-white hover:text-[#C9A227] transition-all duration-300">
                        <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg>
                    </button>
                    ...
                    <button onClick={() => handleShare("tumblr")} className="w-10 h-10 flex items-center justify-center rounded-full bg-[#0088cc] text-white hover:-translate-y-1 hover:shadow-lg transition-all duration-300"><FaTelegramPlane size={18} /></button>
                    ...
                    {(article.advocates && article.advocates.length > 0 ? article.advocates : ([{ name: article.advocateName }] as Advocate[])).map((adv, idx) => {
                        ...
                        return (
                            <div key={idx} className="flex items-center gap-4 p-5 bg-gray-50 rounded-2xl border border-gray-100 hover:border-gray-200 transition-colors">
                                <div className="h-14 w-14 rounded-full bg-[#0A2342] text-[#C9A227] flex items-center justify-center text-2xl font-bold ring-4 ring-[#C9A227]/20 shadow-sm shrink-0 overflow-hidden relative">
                                    {adv?.userId && advocateData[adv.userId]?.photo ? (
                                        <Image src={advocateData[adv.userId].photo} alt={adv.name || "Advocate"} fill className="object-cover" />
                                    ) : (
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        {adv?.userId && advocateData[adv.userId]?.username ? (
                                            <Link href={`/profile/${advocateData[adv.userId].username}`} className="hover:text-[#C9A227] transition-colors">
                                        ...
                                        <Link key={rec.id} href={`/news/${rec.slug}`} className="block group">
                                            <div className="flex gap-4">
                                                <div className="relative w-24 h-24 flex-shrink-0 rounded overflow-hidden bg-gray-100">
                                                    <Image src={getSafeImageUrl(rec.thumbnail)} alt={rec.title} fill sizes="96px" className="object-cover" />
                                                </div>
                                                <div className="flex-1">
