...
import { ArrowLeft, Download, Share2, Printer, Landmark, Scale, Info, Users, FileText, Calendar, Link as LinkIcon, History, User, Gavel } from "lucide-react";
        ...
        <div className="min-h-screen bg-gray-100 pb-16 font-serif text-gray-900 leading-relaxed">
            ...
            <style dangerouslySetInnerHTML={{
                ...
                @media print {
                    ...
                    /* High contrast and background removal */
            ...
            <div className="max-w-5xl mx-auto mt-8 bg-white shadow-2xl border border-gray-300 print:shadow-none print:border-none print:mt-0 px-10 py-12 md:px-20 md:py-20 relative overflow-hidden">
                ...
                {/* Watermark / Logo Placeholder (Subtle) */}
                <div className="absolute top-10 right-10 opacity-[0.03] pointer-events-none select-none">
                    <Landmark size={240} className="text-black" />
                </div>

                {/* Neutral Citations (Top Right) */}
                ...
                {/* Subject / Headnote Metadata Block */}
                ...
                <div className="mb-12 border-y-2 border-double border-gray-200 py-10 px-4 group">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-sm">
                        <div className="space-y-4">
                            ...
                            {judgment.counselDetails?.intervenorCounsel && (
                                <div>
                                    <span className="font-sans font-bold text-[10px] uppercase text-gray-400 block mb-1">For Intervenors:</span>
                                    <p className="font-bold">{judgment.counselDetails.intervenorCounsel}</p>
                                </div>
                            )}
                        </div>
                        <div className="space-y-4">
                            ...
                            {judgment.counselDetails?.stateCounsel && (
                                <div>
                                    <span className="font-sans font-bold text-[10px] uppercase text-gray-400 block mb-1">For State / Union:</span>
                                    <p className="font-bold">{judgment.counselDetails.stateCounsel}</p>
                                </div>
                            )}
                        </div>
                    </div>
                    {(judgment.amicusCuriae || judgment.intervenors) && (
                        <div className="mt-8 border-t border-gray-50 pt-6 text-sm italic text-gray-500 font-sans">
                            {judgment.amicusCuriae && <p className="mb-1">Amicus Curiae: {judgment.amicusCuriae}</p>}
                            {judgment.intervenors && <p>Other Intervenors: {judgment.intervenors}</p>}
                        </div>
                    )}
                </div>
            ...
            {/* RECORD INFORMATION & SIDE DATA (The "Input Fields" metadata) - SEPARATE FROM DOCUMENT */}
            <div className="print-hidden max-w-5xl mx-auto mt-8 mb-20 px-10 md:px-20 py-12 bg-white rounded-3xl border border-gray-200 shadow-xl ">
                ...
                {judgment.discoverySocialInfo && (
                    <div className="mt-8 flex items-center gap-4 text-xs font-sans font-bold text-gray-400 p-4 border-2 border-dotted border-gray-200 rounded-xl">

                        <span>SOCIAL/DISCOVERY TAGS: {judgment.discoverySocialInfo}</span>
                    </div>
                )}
            </div>
        </div>
