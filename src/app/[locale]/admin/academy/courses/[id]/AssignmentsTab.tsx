...
export default function AssignmentsTab({ courseId }: { courseId: string }) {
  ...
  const handleGrade = async () => {
    ...
    try {
      ...
      // Update local state by merging to keep nested relations (like assignment)
                ...
                <div className="space-y-5 flex-1">
                  <div>
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => setGradingStatus('verified')} 
                        disabled={selectedSub.status !== 'pending'}
                        className={`flex-1 py-2 text-sm font-bold rounded-lg border transition disabled:opacity-60 disabled:cursor-not-allowed ${gradingStatus === 'verified' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                      >Verify</button>
                      <button 
                        onClick={() => setGradingStatus('rejected')} 
                        disabled={selectedSub.status !== 'pending'}
                        className={`flex-1 py-2 text-sm font-bold rounded-lg border transition disabled:opacity-60 disabled:cursor-not-allowed ${gradingStatus === 'rejected' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                      >Reject</button>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 resize-none text-sm disabled:bg-gray-50 disabled:text-gray-500"
                    />
                  </div>
                </div>
                ...
                <div className="pt-6 mt-4 border-t border-gray-100">
                  {selectedSub.status === 'pending' ? (
                    ...
                    >
                      ...
                      {gradingStatus === 'verified' ? 'Accept Assignment' : 'Reject & Notify'}
                    </button>
                  ) : (
                    <div className="w-full py-3 px-4 rounded-xl flex flex-col justify-center items-center gap-1 bg-gray-100 border border-gray-200">
                      <span className="font-bold text-gray-600 flex items-center gap-2">
                        {selectedSub.status === 'verified' ? <CheckCircle size={18} className="text-green-600" /> : <XCircle size={18} className="text-red-600" />}
                        Already {selectedSub.status === 'verified' ? 'Verified' : 'Rejected'}
                      </span>
                      <span className="text-xs text-gray-500">Wait for student to re-upload to grade again.</span>
                    </div>
                  )}
                </div>
              </div>
